...
import { binPath, buildHooks, setupText } from "../src/setup.js";
...
describe("binPath", () => {
  it("resolves an absolute path to the compiled index.js next to this module", () => {
    const p = binPath();
    expect(p.startsWith("/")).toBe(true);
    expect(p.endsWith("/index.js")).toBe(true);
  });
});

describe("setupText", () => {
  it("documents MCP registration and the hooks block", () => {
    const text = setupText();
    expect(text).toContain("claude mcp add mac-awake");
    expect(text).toContain("keep-awake --ttl 900");
    expect(text).toContain("on-notification");
    expect(text).toContain("on-stop");
    // The node binary and bin path are embedded as quoted absolute paths.
    expect(text).toContain(JSON.stringify(process.execPath));
    expect(text).toContain(JSON.stringify(binPath()));
  });
});
